...
public class GradeController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<GradeResponseDTO> getGradeById(@PathVariable int id) {
        Optional<Grade> gradeOpt = gradeService.getGradeById(id);
        return gradeOpt.map(grade -> ResponseEntity.ok(convertToResponseDTO(grade)))
                .orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<GradeResponseDTO> createGrade(@RequestBody GradeRequestDTO dto) {
        Optional<Grade> gradeOpt = gradeService.createGrade(dto.getScore(), dto.getStudentId(), dto.getCourseId());
        return gradeOpt.map(grade -> ResponseEntity.status(201).body(convertToResponseDTO(grade)))
                .orElseGet(() -> ResponseEntity.badRequest().build());
    }

    @PutMapping("/{id}")
    public ResponseEntity<GradeResponseDTO> updateGrade(@PathVariable int id, @RequestBody GradeRequestDTO dto) {
        Optional<Grade> gradeOpt = gradeService.updateGrade(id, dto.getScore(), dto.getStudentId(), dto.getCourseId());
        return gradeOpt.map(grade -> ResponseEntity.ok(convertToResponseDTO(grade)))
                .orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    private GradeResponseDTO convertToResponseDTO(Grade grade) {
        ...
        if (grade.getStudent() != null) {
            ...
            // ÖNEMLİ: Student'ın department bilgisini set et
            ...
            // ÖNEMLİ: Student'ın profile bilgisini set et
        ...
        if (grade.getCourse() != null) {
            ...
            // ÖNEMLİ: Course'un teacher bilgisini set et
            if (grade.getCourse().getTeacher() != null) {
                ...
                if (grade.getCourse().getTeacher().getDepartment() != null) {
                    ...
                    tdDto.setStudents(List.of());  // Sonsuz döngüyü kır
                    tdDto.setTeachers(List.of());  // Sonsuz döngüyü kır
